...
public class LoadGroupMessagesService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        if (intent != null) {
            ...
            if(groupID != -1) {
                try {
                    InputStream messagestream = loadMessages();

                    MessageParser parser = new MessageParser();
                    ArrayList<Message> messages = parser.parse(messagestream);

                    for (Message m : messages) {
                        //Log.d("LoadMessagesService", m.getMessageID() + "|" + m.getUserID() + "|" + m.getGroupID() + "   " + m.getMessage());
    ...
    private InputStream loadMessages() throws IOException {
        SharedPreferences pref = this.getSharedPreferences(LoginManager.PREF_NAME, 0);

        URL url = new URL(MenuActivity.SERVER_URI+"Groups/"+groupID+"/messages/");
